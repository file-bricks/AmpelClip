...
- **Microsoft Store Readiness & MSIX-Packaging-Vorbereitung:**
  - Partner-Center Publisher-DN `CN=52596601-BAB4-4F3F-B182-E8F3F273B202` (Lukas Geiger) und Identität `Geiger.AmpelClip` in `store_package.json` und `store_settings.json` synchronisiert.
  - Microsoft Store AppxManifest (`store_package/AmpelClip/AppxManifest.xml`) mit VisualElements, Capabilities (`runFullTrust`) und zweisprachigen Ressourcen (de-DE, en-US) erstellt.
  - Sämtliche Store Tile-Icons (44x44, 50x50, 150x150, 310x150, 310x310) in `store_package/AmpelClip/assets/`, `store_package/AmpelClip/icons/`, `store_assets/` und `assets/icons/` generiert.
  - 4 offizielle Full-HD Store-Screenshots (1920x1080) unter `screenshots/store/` und `README/screenshots/store/` hinterlegt.
  - `STORE_LISTING.md` auf Store Policy 10.1.3 (exakt 7 Suchbegriffe pro Sprache, keine geschützten Drittmarken) harmonisiert.
  - Neue Testsuite `tests/test_store_materials.py` (11 Tests) und erweiterte `scripts/check_store_readiness.py` Preflight-Prüfung (14/14 Checks grün) implementiert. Testsuite: 69/69 passed (100% grün).
